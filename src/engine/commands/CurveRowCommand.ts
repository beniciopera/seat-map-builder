...
export class CurveRowCommand implements Command {
  ...
  execute(): void {
    this.applyCurve(this.newCurveRadius, this.newSeatPositions);
  }

  undo(): void {
    this.applyCurve(this.oldCurveRadius, this.oldSeatPositions);
  }

  private applyCurve(curveRadius: number, seatPositions: Map<ElementId, Point>): void {
